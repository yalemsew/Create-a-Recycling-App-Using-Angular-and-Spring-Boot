...
export class CartComponent implements OnInit {
  ...
  increaseQuantity(item: CartItem) {
    this.cartItemsService.updateUserCartItem(
      this.user.id.toString(), item.product.id.toString(), item.quantity + 1).subscribe((res: any) => {
      console.log(res)
      this.getItems()
    })
  }
  ...
  decreaseQuantity(item: CartItem) {
    if (item.quantity - 1 <= 0) {
      this.cartItemsService.deleteUserCartItem(this.user.id.toString(), item.product.id.toString()).subscribe((res: any) => {
        console.log(res)
        this.getItems()
      })
    } else {
